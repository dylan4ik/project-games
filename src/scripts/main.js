...
document.getElementById('team-next').onclick = function() {
    current = current + 1;
    if (current >= members.length) current = 0;
    showMember();
};

function showMember() {
    document.getElementById('member-name').textContent = members[current].name;
    document.getElementById('member-info').textContent = members[current].info;
    document.getElementById('member-photo').src = members[current].photo;
}
